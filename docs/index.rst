...
Instructions to run the full set of suites, including this one, are available in the `C3S Technical Service Document LINK Pending <>`_ and the `marine observation suit manual LINK PENDING <>`_. However, the purpose of this documentation is to facilitate the understanding of the obs-suit manual by covering the following topics:

- The update of old code paths and repositories found in the marine observation suit manual.
- Explain how to deploy and run the obs-suit code in the `JASMIN cluster services <https://help.jasmin.ac.uk/category/158-getting-started>`_
- Explain how to create and integrate a new `mdf_reader schema <https://glamod.github.io/mdf_reader_documentation/>`_ and `common data model mapper <https://glamod.github.io/cdm_mapper_documentation/>`_ to the **obs-suit** code for a specific data collection from `ICOADS <https://icoads.noaa.gov/>`_ (e.g. a new **SID-DCK**).
